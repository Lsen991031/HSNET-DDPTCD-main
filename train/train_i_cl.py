...
def train_task(args, age, current_task, current_head, class_indexer, model_old=None, prefix=None):
    ...
    if age > 0 and args.exemplar:
        exemplar_dict = load_exemplars(args, torch.device('cpu'))
    ...
    if age > 0:
        ...
        print(ckpt_path)
        sd = torch.load(ckpt_path, map_location=device) # map_location=device)# map_location=torch.device('cpu'))
        sd = sd['state_dict']
        state_dict = dict()
        for k, v in sd.items():
            state_dict[k[7:]] = v
        ...
        sd = torch.load(ckpt_path, map_location=device) # map_location=device)# torch.device('cpu'))#
        print("train_i_cl.device is : {}".format(device))
