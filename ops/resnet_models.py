...
class ResNet(nn.Module):
    ...
    def extract_feat_res(self, x1):
        with torch.no_grad():
            ...
            x1 = self.maxpool.forward(x1)

            t = 0
            for hid, (bid, lid) in enumerate(zip(bottleneck_ids, lids)):
                # ipdb.set_trace()
                res = x1
                x1 = self.__getattr__('layer%d' % lid)[bid].conv1.forward(x1)
                t = t + 1
                # print(t)
                # print(self.__getattr__('layer%d' % lid)[bid])
                x1 = self.__getattr__('layer%d' % lid)[bid].bn1.forward(x1)
                x1 = self.__getattr__('layer%d' % lid)[bid].relu.forward(x1)
                x1 = self.__getattr__('layer%d' % lid)[bid].conv2.forward(x1)
                x1 = self.__getattr__('layer%d' % lid)[bid].bn2.forward(x1)
                x1 = self.__getattr__('layer%d' % lid)[bid].relu.forward(x1)
                x1 = self.__getattr__('layer%d' % lid)[bid].conv3.forward(x1)
                x1 = self.__getattr__('layer%d' % lid)[bid].bn3.forward(x1)

                if bid == 0:
                    res = self.__getattr__('layer%d' % lid)[bid].downsample.forward(res)
                ...
                if lid == 1 and bid == 2:
                    x1 = self.layer1_importance(x1)
                if lid == 2 and bid == 3:
                    x1 = self.layer2_importance(x1)
                if lid == 3 and bid == 5:
                    x1 = self.layer3_importance(x1)
                if lid == 4 and bid == 2:
                    x1 = self.layer4_importance(x1)

        return feats

    def extract_feat_res_ucf(self, x1):
        with torch.no_grad():
